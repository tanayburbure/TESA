...
function Nav() {
  ...
  // We no longer need a ref for the image
  // const imgRef = useRef(null); 

  useEffect(() => {
    // Animate the title element directly
    gsap.to(titleRef.current, {
      x: '-100%', // Animate it completely out of view
      opacity: 0,
      ease: 'power2.inOut',
      scrollTrigger: {
        trigger: "body", // Use a more general trigger like the body
        start: 'top -10%',
        end: 'top -30%',
        scrub: 2,
      },
  ...
  return (
    <div className='fixed w-screen pt-16 px-8 z-[9999]'>
      ...
      <img 
  className='fixed top-8 left-8 w-12 select-none logo-image' // <-- Add 'logo-image'
  src='/images/tesahalf.png' 
  alt='TESA Logo' 
/>

      {/* Title - In its own positioned container */}
      <div className='fixed top-8 left-[90px] h-12 overflow-hidden'>
        <h2 
          ref={titleRef} 
          className='select-none font-[font1] text-zinc-900 font-semibold text-[4.4vh]'
        >
          TESA
        </h2>
      </div>
