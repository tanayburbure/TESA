...
const Team = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end start"], // Pinning effect
  });

  // Convert vertical scroll into horizontal movement
  const xMove = useTransform(scrollYProgress, [0, 1], ["0%", "-63%"]);

  return (
    <div className="wrapper relative bg-black overflow-hidden">
      <motion.div
        ref={sectionRef}
        className="relative h-[300vh] w-full" // Section height increased to enable scrolling
      >
        <motion.div
