...
const ThreeScene = () => {
  ...
  const cameraRef = useRef(
    new THREE.PerspectiveCamera(75, window.innerWidth / (window.innerHeight * 1.1), 0.1, 100)
  );
  ...
  useEffect(() => {
    ...
    renderer.setSize(window.innerWidth, window.innerHeight * 1.1); // Increased height slightly
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    renderer.domElement.style.position = "absolute";
    renderer.domElement.style.top = "0";
    canvasRef.current.appendChild(renderer.domElement);

    // Keep the same visual appearance for 3D element
    const geometry = new THREE.IcosahedronGeometry(1.2, 50); // Adjusted slightly for consistency
    ...
    cameraRef.current.position.z = 2.8; // Slightly adjusted to compensate for increased height
