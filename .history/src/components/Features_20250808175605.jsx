...
const Card = ({ className, children }) => (
  <div
    className={cn(
      "rounded-2xl h-20 w-full text-black p-4 overflow-hidden bg-black border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20",
      className
    )}
  >
...
const CardTitle = ({ children, className }) => (
  <h4 className={cn("text-zinc-800 font-bold tracking-wide mt-4", className)}>{children}</h4>
);

const CardDescription = ({ children, className }) => (
  <p className={cn("mt-2 text-zinc-400 tracking-wide leading-relaxed text-sm", className)}>{children}</p>
);
