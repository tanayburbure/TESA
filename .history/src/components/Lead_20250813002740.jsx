...
const HeroParallax = ({ products }) => {
    ...
    // Modified scroll behavior - cards scroll horizontally until end, then page scrolls down
        ...
        >
            ...
            >
                ...
                <motion.div 
                    ...
                    style={{ x: translateX }}
                >
                    {/* CHANGE 1: Added 'index' to map function to identify the first card */}
                    {products.map((product, index) => (
                        <ProductCard
                            product={product}
                            key={product.title}
                            /* CHANGE 2: Conditionally apply a style to the first card (index === 0).
                                'translateX(-40vh)' moves the card left by 40% of the viewport height.
                                Using 'transform' ensures it doesn't disrupt the spacing of other cards.
                            */
                            style={index === 0 ? { transform: "translateX(-40vh)" } : {}}
                        />
                    ))}
                </motion.div>
            </motion.div>
...
// Header component - keeping all texts outside cards
const Header = () => {
    return (
...
// CHANGE 3: Modified ProductCard to accept and apply a 'style' prop
const ProductCard = ({ product, style }) => {
    return (
        <motion.div
            style={style} // The passed-in style is applied here
            whileHover={{
                y: -20,
            }}
            key={product.title}
