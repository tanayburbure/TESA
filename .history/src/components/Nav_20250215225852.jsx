  ...
  useEffect(() => {
    gsap.to(titleRef.current, {
      ...
      onUpdate: function () {
        ...
        if (progress > 0.3) {
          gsap.to(titleRef.current, { opacity: 0, clipPath: 'inset(0% 100% 0% 0%)', duration: 0.3 });
        } else {
          gsap.to(titleRef.current, { opacity: 1, clipPath: 'inset(0% 0% 0% 0%)', duration: 0.3 });
        }
      }
    });
  }, []);
  ...
  return (
    <div className='flex relative justify-between items-center pt-8 px-8'>
      <div className='flex items-center px-3 z-[99999999] py-[2.2vh]'>
        ...
        <h2 ref={titleRef} className='select-none fixed text-zinc-200 pl-20 text-2xl'>
          TESA
        </h2>
      </div>
