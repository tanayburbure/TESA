...
const ExperienceCard = memo(function ExperienceCard({ name, job, paragraph }) {
  ...
  return (
    <motion.div
      ref={cardRef}
      className="w-full min-h-[22vh] sm:min-h-[24vh] flex items-center bg-gray-100 overflow-hidden p-4 sm:p-6 mb-4 rounded-xl"
      layout
      role="region"
      aria-label={name}
    >
      <motion.div ref={textRef} className="w-full" layout>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h2 className="text-xl sm:text-2xl font-bold leading-tight">{name}</h2>
        </div>
