...
const HeroParallax = ({ products }) => {
    ...
    return (
        <div
            ref={ref}
            className="h-[360vh] select-none py-40 overflow-hidden antialiased relative flex flex-col self-auto [perspective:1000px] [transform-style:preserve-3d] "
        >
            <Header />
            <motion.div
                style={{
    ...
    return (
        ...
        >
            ...
            <div className="absolute bottom-[42%] left-6 right-4">
                <h2 className="text-2xl font-[font4] font-bold text-zinc-500 p-2 rounded">
                    {product.title}
                </h2>
            </div>
            ...
            <div className="absolute top-[56%] p-4 rounded-xl group-hover/product:opacity-100 text-white w-full ">
                <div className="flex items-center mb-2">
                    ...
                    <div>
                        <p className="text-lg font-[font3] text-gray-300 font-bold">{product.profile.name}</p>
                        <p className="text-sm font-[font2] text-gray-500">{product.profile.details}</p>
                    </div>
                </div>
                <p className="text-sm font-[font3] tracking-tight mt-3 text-gray-400">{product.description}</p>
            </div>
        </motion.div>
    );
};
