...
const HeroParallax = ({ products }) => {
    ...
    // Spacer height so scrolling distance matches horizontal length
    const extraScrollWidth = products.length * 36 - 100; // in rem - approx viewport
    const spacerHeight = products.length * 35; // controls scroll range
    ...
    // Horizontal movement mapped to vertical scroll progress
    const translateX = useSpring(
        useTransform(scrollYProgress, [0, 1], [0, -extraScrollWidth * 16]), // rem → px
        springConfig
    );

    // Keep your original perspective + tilt animations
    ...
    const translateY = useSpring(
        useTransform(scrollYProgress, [0, 0.1, 0.9, 1], [0, 0, 0, 0]), // stays fixed vertically
        springConfig
    );
    ...
    return (
        <section ref={containerRef} className="relative h-[100vh]">
            ...
            {/* Invisible scroll space to allow horizontal movement */}
