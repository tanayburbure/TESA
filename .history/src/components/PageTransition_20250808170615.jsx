  ...
  return (
    ...
    >
          ...
          {content.map((item, index) => (
            <div key={`${item.title}-${index}`} className="my-20">
              ...
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: activeCard === index ? 1 : 0.3 }}
                className="text-lg mt-4 max-w-sm text-slate-300 transition-opacity duration-300"
              >
                {item.description}
              </motion.p>
            </div>
      ...
      <div
        ...
        className={cn(
          "sticky top-10 hidden h-60 w-80 overflow-hidden rounded-md bg-white p-4 shadow-lg lg:block",
          contentClassName
        )}
      >
        {content.length > 0 && content[activeCard]?.content ? (
          <div className="w-full h-full overflow-auto">{content[activeCard].content}</div>
        ) : null}
      </div>
    </motion.div>
  );
