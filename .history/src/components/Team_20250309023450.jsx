...
const Team = () => {
  ...
  const grayscale = useMemo(() => {
    return useTransform(scrollY, [0, window.innerHeight], [100, 0], {
      clamp: true,
    });
  }, [scrollY]);
  ...
  useEffect(() => {
    ...
    // Delay the GSAP initialization slightly to ensure DOM is fully ready
    const initAnimation = () => {
      ctx = gsap.context(() => {
        if (sectionRef.current && imageContainerRef.current) {
          // Horizontal scrolling animation with better performance settings
  ...
  // Memoize the team images to prevent unnecessary re-renders
    ...
    return teamImages.map((image, index) => (
      <motion.div
        ...
        transition={{ 
          duration: 0.8, 
          ease: "easeOut",
          layout: { duration: 0 } // Disable layout animations for better performance
        }}
        style={{ 
          filter: grayscale,
          willChange: "transform, opacity" // Hint to browser for optimization
        }}
        layoutId={`team-image-${index}`} // Stable layout ID
      >
        ...
        <img
          ...
          loading="lazy"
          decoding="async" // Helps with image decoding performance
        />
      </motion.div>
    ));
  }, [grayscale]);
