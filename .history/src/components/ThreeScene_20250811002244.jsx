...
const ThreeScene = () => {
  ...
  // Function to set initial position based on current scroll
  const setInitialPosition = useCallback(() => {
    if (!sphereRef.current || initialPositionSet.current) return;
    
    const scrollY = window.scrollY;
    sphereRef.current.position.y = 0.8 - scrollY * 0.0010;
    
    // Set initial color based on scroll position
    let initialColor;
    if (scrollY < 500) {
      initialColor = 0;
    } else if (scrollY < 800) {
    ...
    colorChangeRef.current = initialColor;
    material.uniforms.uColorChange.value = initialColor;
  ...
  useEffect(() => {
    // Reset position flag on component mount
    ...
    sphereRef.current.position.set(0, 0.4, 0); // Default position
    sceneRef.current.add(sphereRef.current);
    
    cameraRef.current.position.z = 3.25;

    // Set initial position based on current scroll position after a brief delay
    ...
    // ScrollTrigger with refresh to handle initial scroll position
    ...
    const scrollAnimation = gsap.fromTo(
      ...
      { opacity: 0 },
      {
        ...
        scrollTrigger: {
          ...
          onRefresh: () => {
            // Ensure position is set correctly on ScrollTrigger refresh
            setInitialPosition();
          }
        },
      }
    ...
    <div className="w-full">
      <div className="landing w-screen relative flex flex-col">
        <div className="flex items-center justify-center">
          <h1
            ref={h1Ref}
            className="text-[7vh] font-[font4] text-center absolute font-semibold tracking-tight pt-[65vh] z-[1]"
          >
