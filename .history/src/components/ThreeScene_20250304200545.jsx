...
const ThreeScene = () => {
  ...
  const h2Ref = useRef(null); // Added h2Ref for the h2 element
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Move h2 element and make it vanish after moving 30%
      const h2PositionY = 12 - (scrollY * 0.001 * 0.3); // Adjust the multiplier for movement
      gsap.to(h2Ref.current, {
        y: h2PositionY,
        opacity: scrollY > 30 ? 0 : 1, // Vanish after moving 30%
        duration: 0.3,
      });

      // Move h2 tag like a 3D element
      gsap.to(h2Ref.current, {
        y: h2PositionY,
        z: -scrollY * 0.01, // Adding a 3D effect
        duration: 0.3,
      });
    };
