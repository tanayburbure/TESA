...
const HeroParallax = ({ products }) => {
    ...
    const translateX = useSpring(
        useTransform(scrollYProgress, [0, 0.8, 1], [2100, -1000, -100]),
        springConfig
    );
    ...
    // --- MODIFIED CODE ---
    // This new definition for `translateY` creates the pinned effect.
    // It keeps the vertical position constant between 20% and 80% of the scroll progress.
    const translateY = useSpring(
        useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [-600, 100, 100, 1000]),
        springConfig
    );
    ...
    return (
        <div
            ref={ref}
            className="h-[160vh] w-[200vw] select-none py-40 overflow-hidden antialiased relative flex flex-col [perspective:1000px] [transform-style:preserve-3d]"
        >
            <Header />
