...
const CardTitle = ({ children, className }) => (
  <h4 className={cn("text-zinc-900 text-[3vh] font-bold tracking-wide mt-1 font-[font2]", className)}>{children}</h4>
);

const CardDescription = ({ children, className }) => (
  <p className={cn("mt-2 text-zinc-600 tracking- text-[2vh] font-regular font-[font3] leading-5", className)}>{children}</p>
);
        ...
        {features.map(({ title, description, icon, buttonLabel, link }, idx) => (
          ...
          >
            ...
            <Card className="bg-white text-zinc-900 h-60">
              ...
              {buttonLabel && (
                <button className="flex items-center justify-center border border-zinc-700 rounded-full px-5 py-1 mt-4">
                  {buttonLabel}
                </button>
              )}
            </Card>
