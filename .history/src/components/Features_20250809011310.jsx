...
const Card = ({ className, children }) => (
    <div
        className={cn(
            "rounded-2xl h-[30vh] w-76 p-4 overflow-hidden bg-black border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20",
            className
        )}
    >
...
const CardTitle = ({ children, className }) => (
    <h4
        className={cn(
            "text-zinc-900 text-[3vh] font-bold tracking-wide mt-1 font-[font2]",
            className
        )}
    >
        {children}
...
const CardDescription = ({ children, className }) => (
    <p
        className={cn(
            "mt-2 text-zinc-600 tracking- text-[2vh] font-regular font-[font3] leading-5",
            className
        )}
    >
        {children}
    ...
    return (
        <div className="flex flex-col h-[92vh] items-center pt-24">
            ...
            <h3 className="text-center text-[2.5vh] font-[font3] -mt-2 max-w-4xl">
                Our website is equipped with highly impactful features, thoughtfully
                designed to enhance student experience and usability.
            </h3>

            <div className="no-draw grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-24 w-full max-w-6xl px-8">
                {features.map(
                    ({ title, description, icon, buttonLabel, link }, idx) => (
                        <a
                            href={link}
                            key={link ?? idx}
                        ...
                        >
                            ...
                            <Card className="bg-white text-zinc-900 h-60">
                                <div className="flex items-center gap-2 ">
                                    <CardTitle className="text-lg font-semibold  flex-1 -mt-2">
                                        {title}
                                    </CardTitle>
                                    {icon && (
                                        <div className="text-[3.1vh] text-[#3B6654]">{icon}</div>
                                    )}
                                </div>
                                ...
                                {buttonLabel && (
                                    <button
                                        className="border border-zinc-700 rounded-full ml-20 px-5 py-1 mt-4 bg-[#3B6654] shadow-[0_10px_20px_-10px_#3B6654] text-zinc-200 cursor-pointer font-semibold text-[16px] leading-6 select-none w-fit">
                                        {buttonLabel}
                                    </button>
                                )}
                            </Card>
