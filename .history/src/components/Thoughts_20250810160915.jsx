...
const Thoughts = ({
  ...
  const intervalRef = useRef(null); // Add ref to track interval
  ...
  const handleNext = () => {
    setActive((prev) => (prev + 1) % testimonials.length);
    resetTimer(); // Reset timer when button is clicked
  };
  ...
  useEffect(() => {
    resetTimer(); // Start timer on mount and when dependencies change
  ...
  // ONLY CHANGE: Replace randomRotateY with this function
  const getCardAngle = (index) => {
    const baseAngles = [-8, -4, 0, 4, 8, -6, 2];
  ...
  return (
    <div className="mx-auto select-none h-[75vh] mt-8 max-w-sm px-4 font-sans antialiased md:max-w-4xl md:px-8 lg:px-12">
              ...
              {testimonials.map((testimonial, index) => (
                <motion.div
                  ...
                  initial={{
                    ...
                    scale: 0.9,
                    z: -100,
                    rotate: getCardAngle(index), // Changed from randomRotateY()
                  }}
                  animate={{
                    opacity: isActive(index) ? 1 : 0.7,
                    scale: isActive(index) ? 1 : 0.95,
                    z: isActive(index) ? 0 : -100,
                    rotate: isActive(index) ? 0 : getCardAngle(index), // Changed from randomRotateY()
