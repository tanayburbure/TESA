...
const Card = ({ className = "", children }) => (
  <div
    className={cn(
      "rounded-2xl p-4 overflow-hidden bg-black border border-transparent dark:border-white/[0.2] relative z-20 group-hover:border-slate-700",
      className
    )}
  >
...
const CardTitle = ({ children, className = "" }) => (
  <h4 className={cn("text-zinc-900 text-[3vh] font-bold tracking-wide mt-1 font-[font2]", className)}>
    {children}
  </h4>
);

const CardDescription = ({ children, className = "" }) => (
  <p className={cn("mt-2 text-zinc-600 text-[2vh] font-regular font-[font3] leading-5", className)}>
    {children}
  </p>
);
...
const WebFeature = () => {
  ...
  return (
    <div className="flex flex-col select-none h-[100vh] items-center pt-24">
        ...
        {features.map(({ title, description, icon, buttonLabel, link }, idx) => (
          ...
          >
            ...
            <Card className="bg-white text-zinc-900 h-60">
              ...
              {buttonLabel && (
                <button className="border border-zinc-700 rounded-full ml-20 px-5 py-1 mt-4 bg-[#3B6654] shadow-[0_10px_20px_-10px_#3B6654] text-zinc-200 cursor-pointer font-semibold text-[16px] leading-6 select-none w-fit">
                  {buttonLabel}
                </button>
              )}
            </Card>
