...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollY < 200) {
        setColorChange(0); // Red
      } else if (scrollY < 500) {
        setColorChange(1); // Yellow
      } else if (scrollY < 800) {
        setColorChange(2); // Blue
      } else {
        setColorChange(3); // Dark Gray
      }
    };
    ...
    <div className="w-full">
      <div className="landing w-screen relative flex flex-col">
        <div className="flex items-center justify-center">
          <h2
            ref={h2Ref}
            className={`text-[2.5vw] z-[99] font-bold text-center absolute cursor-pointer pt-4 transition-colors duration-500 ${
              colorChange === 0
                ? "text-red-500"
                : colorChange === 1
                ? "text-yellow-500"
