...
export default function HamburgerMenu() {
  ...
  const getHorizontalLineAnimation = (position) => ({
    ...
    scale: isHovered ? 0 : 1, // Change scale to 0 for a more direct shrink effect
    rotateZ: 0,
    width: isHovered ? "0px" : "24px", // Change width to 0px
    height: "2px",
    filter: `blur(${isHovered ? 0 : 0}px)`, // Remove the blur effect
  ...
  const getVerticalLineAnimation = (position) => ({
    ...
    scale: isHovered ? 1 : 0, // Change scale to 0
    rotateZ: 0,
    width: "2px",
    height: isHovered ? "24px" : "0px", // Change height to 0px
    filter: `blur(${isHovered ? 0 : 0}px)`, // Remove the blur effect
    x: isHovered ? (position === 'left' ? -6 : position === 'right' ? 6 : 0) : 0, // Animate x from 0
  });
