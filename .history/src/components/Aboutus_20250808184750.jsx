...
function Aboutus() {
  ...
  const buttonRef = useRef(null); // Ref for the first button

  useEffect(() => {
    const paragraph = paragraphRef.current;
    const button = buttonRef.current;

    // Wrap each character in a span inside paragraph
    paragraph.innerHTML = paragraph.textContent
      .split("")
      .map((char) => `<span>${char}</span>`)
      .join("");

    // Animate spans on scroll (paragraph)
    ...
    // Animate the button on scroll (example animation)
    gsap.fromTo(
      button,
      { opacity: 0.5, color: "#3f3f46" },
      {
        opacity: 1,
        color: "#fafafa",
        scrollTrigger: {
    ...
    <div className="h-[100vh] relative flex bg-[#EBEBEB]">
      ...
      <div className="absolute px-10 text-center flex flex-col items-center justify-center">
        <div className="flex flex-col items-center mt-32">
          <p
            ref={paragraphRef}
            className="w-[76%] text-[2.7vh] z-[1] mt-[27vh] tracking-tight leading-[3.7vh] text-zinc-400"
          >
            The Telecom Engineers Student Association (TESA) has proudly upheld its legacy for many years. <br />
            ...
            <br />
            Each year, students from the ENTC Department, actively take part in TESA’s initiatives. A major highlight of TESA is its annual two-day flagship event, "VERITAS"
          </p>
        </div>
        <br />
        ...
        <div className="mt-8 flex gap-16 z-[1]">
          <button
            ref={buttonRef}
            className="text-zinc-700 border border-zinc-400 px-6 py-3 rounded-full mx-2 hover:bg-zinc-800 transition-all hover:text-zinc-200 duration-300"
          >
            Become a Member
          </button>
