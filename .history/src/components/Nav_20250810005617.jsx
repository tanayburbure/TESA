...
function Nav() {
  ...
  useEffect(() => {
    // Set initial transform to enable hardware acceleration
    ...
    gsap.to(titleRef.current, {
      x: '-70px', // Move toward the image
      y: '0px',
      scale: 0.85, // Shrink slightly as it approaches
      opacity: 0.6,
      duration: 0.8,
      ease: 'power2.out',
      delay: 2,
  ...
  return (
    <div className='fixed w-screen pt-16 px-8 z-[9999]'>
      {/* Logo & Title - REMOVED OVERFLOW CONTAINER */}
      <div className='fixed top-8 left-8 flex items-center'>
        ...
        />
        {/* Removed the overflow: hidden container that was creating the "wall" */}
        <h2 
          ref={titleRef} 
          className='select-none font-[font1] text-zinc-900 font-semibold text-[4.4vh] ml-2'
