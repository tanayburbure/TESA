...
const HeroParallax = ({ products }) => {
    ...
    // GSAP horizontal scrolling effect (simulated with framer-motion)
    useEffect(() => {
        // Note: Original code used GSAP but we'll simulate with scroll-based transforms
        const handleScroll = () => {
            if (cardsContainerRef.current && sectionRef.current) {
                const rect = sectionRef.current.getBoundingClientRect();
                const progress = Math.max(0, Math.min(1, -rect.top / (rect.height - window.innerHeight)));
                const translateX = progress * -70; // -70% as in original
