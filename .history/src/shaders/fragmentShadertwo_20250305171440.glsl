...
void main() {
    ...
    // Modified to generate a good yellow effect similar to the blue one
    vec4 colorYellowModified = vec4(1.0, 0.9608, 0.8706, 1.0); // Adjusted to a vibrant yellow
    vec4 colorYellowModified2 = vec4(1.0, 0.9608, 0.8706, 1.0); // Adjusted to a vibrant yellow

    vec4 finalColor = mix(colorRed, colorYellowModified, smoothstep(0.4, 1.0, uColorChange));
    finalColor = mix(finalColor, colorYellowModified2, smoothstep(1.0, 1.4, uColorChange));
