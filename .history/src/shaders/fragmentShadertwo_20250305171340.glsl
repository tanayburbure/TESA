...
void main() {
    ...
    vec4 c7 = vec4(1.0, 0.9608, 0.8706, 1.0); // Yellow effect color
    vec4 c8 = vec4(1.0, 0.9608, 0.8706, 1.0); // Yellow effect color

    float v = smoothstep(-0.14, 0.14, vElevation);
    vec4 colorRed = mix(c1, c2, v);
    vec4 colorYellow = mix(c3, c4, v);
    vec4 colorBlue = mix(c5, c6, v);
    vec4 colorYellowEffect = mix(c7, c8, v); // Yellow effect color mix
