...
void main() {
            ...
            vec4 c5 = vec4(0.6784, 0.8471, 0.9010, 1.0); // Light blue color
            vec4 c6 = vec4(0.8314, 0.9333, 0.9608, 1.0); // Light blue with white effect

            float v= smoothstep(-.14,.14,vElevation);
            vec4 colorred = mix(c1,c2,v);
            vec4 coloryellow = mix(c3,c4,v);
            vec4 colorblue = mix(c5, c6, uColorChange);
